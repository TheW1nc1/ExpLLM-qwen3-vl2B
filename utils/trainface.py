...
class ModelArguments:
    ...
    ft_model_name_or_path: Optional[str] = field(default=None)
...
class DataArguments:
    ...
    question_index: int = field(default=-1) # 1 for first box + random other, 0 for only box, 2 for random all
...
def train():
    ...
    if model_args.ft_model_name_or_path is not None and os.path.exists(model_args.ft_model_name_or_path):
        # 基于checkpoint 或者 一阶段训好的结果
        ft_model_name = os.path.expanduser(model_args.ft_model_name_or_path)
        model = LocLLMModel.from_pretrained(ft_model_name, use_cache=True)
        tokenizer = transformers.AutoTokenizer.from_pretrained(ft_model_name, use_fast=False, padding_side='right', model_max_length=training_args.model_max_length)
        ...
        # load mm projector weights # TODO how to train mm projector only
        if model_args.pretrain_mm_mlp_adapter is not None:
            print('Load pretrained mm_projector from: ', model_args.pretrain_mm_mlp_adapter)
            mm_projector_weights = torch.load(model_args.pretrain_mm_mlp_adapter, map_location='cpu')
            update_state = {} # TODO check if this is correct, seems name wrong
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        ...
        trainer.train()
    if model_args.save_mm_projector:
        torch.save(model.mm_projector.state_dict(), os.path.join(training_args.output_dir, "mm_projector.pth"))
    trainer.save_state()
