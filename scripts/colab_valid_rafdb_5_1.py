# ========== 复制整段到 Colab 一个单元格运行：RAF-DB FER 测评（5:1 FER+CoT 训练结果 rafdb_expllm_5_1） ==========
# 完全模仿 utils/validfaceEMO.py + eval_metrics.py 逻辑，仅测评 FER
# 与 colab_train_expllm_4aug.py 的 INSTR_FER、template 一致（5:1 与 4aug 同 prompt）
...
print(">>> RAF-DB FER 测评脚本开始 (rafdb_expllm_5_1) <<<", flush=True)
...
MODEL_DIR = os.environ.get("MODEL_DIR", os.path.join(PROJECT_ROOT, "saves/qwen3vl2b/lora/rafdb_expllm_5_1"))
CHECKPOINT = 3688  # 5:1 纯 8 轮最终步；None 时由 CHECKPOINT_POLICY 决定；可试 2450/3000/3200 等对比
CHECKPOINT_POLICY = "latest"  # "earliest"=最早保存的 | "latest"=最新
OUTPUT_DIR = os.environ.get("OUTPUT_DIR", os.path.join(PROJECT_ROOT, "eval_rafdb_5_1"))
...
# 与训练 INSTR_FER 完全一致（train-eval 必须一致）
